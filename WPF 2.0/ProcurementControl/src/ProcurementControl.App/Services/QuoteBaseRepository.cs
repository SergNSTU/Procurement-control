...
LIMIT @limit";
        ...
        while (reader.Read())
        {
            result.Add(new QuoteHistoryRow
            {
                ...
                PN = Text(reader, 3),
                Supplier = Text(reader, 4),
                UnitPrice = NullableDouble(reader, 5),
                LeadTime = Text(reader, 6),
                LeadTimeTotal = Text(reader, 7),
                Mfg = Text(reader, 8),
                IsWinner = !reader.IsDBNull(9) && Convert.ToInt64(reader.GetValue(9)) != 0,
                WinnerReason = Text(reader, 10),
                Warning = Text(reader, 11),
                RfqPath = Text(reader, 12),
                Priority = Text(reader, 13),
            });
        }
        return result;
    }
