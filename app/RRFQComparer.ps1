    ...
    foreach ($row in $suppliers.Rows) {
        ...
        if (-not (Test-PurchaseSupplierErpNotRequired ([string]$row.supplier))) {
            ...
            if (-not (Convert-DbBool $row.erp_roger_sent)) {
                [void]$items.Add([pscustomobject]@{ DealId = [int]$row.deal_id; SupplierId = [int]$row.supplier_id; Severity = 'Info'; Title = "${prefix}: не отправлен ERP Роджеру"; DueDate = ''; Source = 'auto' })
            }
        }
...
function Delete-PurchaseSupplier {
    ...
    Touch-PurchaseDeal $dealId
    Write-ActivityLog 'supplier' $SupplierId 'Удален поставщик' $supplier $dealId
}
...
$where += " AND trim(IFNULL(d.executor, '')) = '' AND (IFNULL(d.status, '') IN ('RFQ', 'PO', 'Закупка', 'В работе') OR (IFNULL(d.status, '') = 'Заказано' AND (IFNULL(stats.supplier_count, 0) = 0 OR IFNULL(stats.invoice_confirmed_count, 0) < IFNULL(stats.supplier_count, 0))))"
    ...
    if ($Filter -ne 'China' -and $Filter -ne 'Tracking') {
        $where += " AND trim(IFNULL(d.assembly_location, '')) <> 'Китай'"
    }
...
function Get-UiColor {
    ...
    switch ($Name) {
        'Canvas' { return [Drawing.Color]::FromArgb(244, 247, 251) }
        'Surface' { return [Drawing.Color]::FromArgb(255, 255, 255) }
        'SurfaceAlt' { return [Drawing.Color]::FromArgb(248, 250, 252) }
        'Line' { return [Drawing.Color]::FromArgb(213, 222, 235) }
        'Header' { return [Drawing.Color]::FromArgb(229, 236, 246) }
        'Text' { return [Drawing.Color]::FromArgb(25, 37, 55) }
        'Muted' { return [Drawing.Color]::FromArgb(91, 111, 136) }
        'Primary' { return [Drawing.Color]::FromArgb(154, 194, 255) }
        'PrimaryText' { return [Drawing.Color]::FromArgb(21, 52, 91) }
        'Secondary' { return [Drawing.Color]::FromArgb(232, 242, 239) }
        'SecondaryText' { return [Drawing.Color]::FromArgb(36, 78, 67) }
        'Nav' { return [Drawing.Color]::FromArgb(30, 41, 59) }
        'NavActive' { return [Drawing.Color]::FromArgb(71, 91, 122) }
        'NavHover' { return [Drawing.Color]::FromArgb(51, 65, 85) }
        'NavText' { return [Drawing.Color]::FromArgb(241, 245, 249) }
        'NavMuted' { return [Drawing.Color]::FromArgb(190, 203, 222) }
        'Success' { return [Drawing.Color]::FromArgb(222, 246, 230) }
        'Warn' { return [Drawing.Color]::FromArgb(255, 245, 223) }
        'Info' { return [Drawing.Color]::FromArgb(230, 239, 255) }
        'Attention' { return [Drawing.Color]::FromArgb(246, 235, 255) }
        'Danger' { return [Drawing.Color]::FromArgb(255, 232, 232) }
...
function Set-GridLook {
    ...
    $Grid.BackgroundColor = Get-UiColor 'SurfaceAlt'
    ...
    $Grid.ColumnHeadersDefaultCellStyle.ForeColor = Get-UiColor 'Text'
    $Grid.ColumnHeadersDefaultCellStyle.SelectionBackColor = Get-UiColor 'Header'
    $Grid.ColumnHeadersDefaultCellStyle.SelectionForeColor = Get-UiColor 'Text'
    ...
    $Grid.DefaultCellStyle.SelectionBackColor = [Drawing.Color]::FromArgb(214, 231, 255)
...
function Set-PrimaryButtonLook {
    ...
    $Button.FlatAppearance.BorderColor = [Drawing.Color]::FromArgb(122, 169, 240)
    $Button.FlatAppearance.MouseOverBackColor = [Drawing.Color]::FromArgb(181, 211, 255)
    $Button.FlatAppearance.MouseDownBackColor = [Drawing.Color]::FromArgb(130, 178, 245)
...
function Set-SecondaryButtonLook {
    ...
    $Button.FlatAppearance.BorderSize = 1
    $Button.FlatAppearance.BorderColor = [Drawing.Color]::FromArgb(178, 212, 202)
    $Button.FlatAppearance.MouseOverBackColor = [Drawing.Color]::FromArgb(218, 235, 229)
    $Button.FlatAppearance.MouseDownBackColor = [Drawing.Color]::FromArgb(198, 224, 216)
...
function Set-NavButtonLook {
    ...
    $Button.FlatAppearance.MouseOverBackColor = Get-UiColor 'NavHover'
    $Button.FlatAppearance.MouseDownBackColor = Get-UiColor 'NavActive'
    $Button.BackColor = $(if ($Active) { Get-UiColor 'NavActive' } else { Get-UiColor 'Nav' })
    $Button.ForeColor = Get-UiColor 'NavText'
    $Button.Font = New-Object Drawing.Font('Segoe UI', 9, $(if ($Active) { [Drawing.FontStyle]::Bold } else { [Drawing.FontStyle]::Regular }))
...
function Set-GroupLook {
    ...
    $Group.BackColor = Get-UiColor 'Canvas'
...
"@ @{ '@needle'='%'+$PN.Trim()+'%' }
    foreach($row in $table.Rows){
        ...
        New-PriceSearchResult 'Наши квоты' ([string]$row.pn) ([string]$row.mfg) ([double]$row.unit_price) '' '' '' ([string]$row.lead_time) ([string]$row.quote_date) ''
    }
}
...
function Get-PriceSearchLcscQuotes {
    ...
    try {
        ...
        foreach($product in @($response.result.dataList)){
            ...
            $lead='В наличии'
            if($null -ne $product.flashSaleProductPO -and $product.flashSaleProductPO.arriveDays){$lead=('{0} д' -f $product.flashSaleProductPO.arriveDays)}
            $date=Get-NowText
...
function Get-PriceSearchMouserQuotes {
    ...
    if([string]::IsNullOrWhiteSpace($apiKey)){
        return @(New-PriceSearchResult 'Mouser' $PN '' '' '' '' '' 'Нужен API-ключ' (Get-NowText) $link)
    }
    try {
        ...
        if($items.Count -eq 0){return @(New-PriceSearchResult 'Mouser' $PN '' '' '' '' '' 'Данные не найдены' (Get-NowText) $link)}
        return @($items)
    } catch {
        return @(New-PriceSearchResult 'Mouser' $PN '' '' '' '' '' 'Ошибка API — открыть сайт' (Get-NowText) $link)
    }
}
function Get-PriceSearchExternalFallback {
    ...
    try {
        ...
        if($null -eq $response -or [int]$response.StatusCode -lt 200 -or [int]$response.StatusCode -ge 400){$status='Открыть сайт'}
    } catch { $status='Открыть сайт' }
...
function Get-PriceSearchGptPrompt {
    param([string]$PN,[int]$Quantity=0)
    return ('Найди цены, наличие, MOQ, Lead Time и поставщиков для PN: {0}, количество: {1}. Не выдумывай данные, указывай источники. Дополнительно предложи возможные аналоги и укажи риски замены.' -f $PN,$Quantity)
}
...
function Show-MainFormV2 {
    ...
    $shell.ColumnCount = 2
    $shell.RowCount = 1
    Add-TableColumnStyle $shell 'Absolute' 220
    ...
    $navPanel.BackColor = Get-UiColor 'Nav'
    $navPanel.ColumnCount = 1
    $navPanel.RowCount = 15
    ...
    $navTitle.ForeColor = Get-UiColor 'NavText'
    ...
    $navPanel.Controls.Add($btnNavReminders, 0, 5)
    ...
    $navPanel.Controls.Add($btnNavQuoteBase, 0, 6)
    ...
    $navPanel.Controls.Add($btnNavCompelParser, 0, 7)
    ...
    $navPanel.Controls.Add($btnNavPriceSearch, 0, 8)
    ...
    $navPanel.Controls.Add($btnNavInstructions, 0, 9)
    ...
    $navPanel.Controls.Add($btnNavSettings, 0, 10)
    ...
    $navPanel.Controls.Add($btnNavNotes, 0, 11)

    $navPanel.Controls.Add($btnNavHistory, 0, 12)
...
$pageHost.Controls.Add($bitrixPage)
    ...
    $btnBitrixRefresh.Add_Click({
        ...
        try {
            $apiUrl = 'https://unirec.bitrix24.ru/rest/165/29uhx17qmtp0nj06/tasks.task.list'
            $queryParams = 'filter[GROUP_ID]=27&filter[>=STATUS]=1&filter[<=STATUS]=6&select[]=id&select[]=title&select[]=deadline&select[]=status&order[ID]=DESC'
            $r = Invoke-RestMethod -Uri "$apiUrl`?$queryParams" -Method Post -TimeoutSec 20
            ...
            foreach ($t in @($tasks)) {
                ...
                $bitrixGrid.Rows.Add($t.id, $t.title, $deadline, [string][char]0x041E + [string][char]0x0442 + [string][char]0x043A + [string][char]0x0440 + [string][char]0x044B + [string][char]0x0442 + [string][char]0x044C) | Out-Null
            }
            $count = @($tasks).Count
    ...
    $filesGroup = New-Object Windows.Forms.GroupBox
    $filesGroup.Text = '1. Входные файлы'
    ...
    $actionsGroup.Text = '2. Настройки и запуск'
    ...
    $previewGroup.Text = '3. Preview и проверка результата'
    ...
    $purchaseLayout.Controls.Add($purchaseToolbar, 0, 0)
    ...
    $purchaseLayout.Controls.Add($purchaseContent, 0, 1)
    ...
    $componentStatusValues = @('В работе', 'Ожидание ответа', 'На отслеживании', 'Заказано', 'Выполнено', 'Не актуально')
    ...
    $btnOpenReminderTarget.Text = 'Открыть сделку'
    ...
    $btnPriceSearch.Text = 'Найти цены'; $btnPriceSearch.Width = 120; $btnPriceSearch.Height = 32; $btnPriceSearch.Margin = New-Object Windows.Forms.Padding(0, 2, 8, 0)
    ...
    foreach($columnInfo in @(@('Source','Источник',125),@('PN','PN',170),@('Manufacturer','Производитель',150),@('Price','Цена',90),@('Currency','Валюта',80),@('MOQ','MOQ',90),@('Stock','Остаток',100),@('LeadTime','Lead Time',120),@('Date','Дата',145),@('Link','Ссылка',330))){
...
'@
    ...
    function Refresh-PurchaseDeals {
        ...
        try {
            ...
            foreach ($row in $table.Rows) {
                ...
                if (Convert-DbBool $row.archived) {
    ...
    function Open-SelectedReminderTarget {
        ...
        if ($null -eq $info -or [int]$info.DealId -le 0) {
            throw 'У этого напоминания нет связанной сделки.'
        }
        ...
        if ($source -eq 'component' -or $source -eq 'task' -or $source -eq 'components') {
            ...
            if ($null -eq $target) { throw 'Связанная сделка не найдена в разделе "Задачи".' }
        ...
        if ($null -eq $target) {
            $previousFilter = $cmbDealFilter.SelectedItem
            $cmbDealFilter.SelectedItem = 'Все включая архив'
            Refresh-PurchaseDeals
            foreach ($gridRow in $dealsGrid.Rows) {
                if ([int]$gridRow.Tag -eq [int]$info.DealId) { $target = $gridRow; break }
            }
            if ($null -eq $target -and $null -ne $previousFilter -and [string]$previousFilter -ne 'Все включая архив') {
                $cmbDealFilter.SelectedItem = $previousFilter
            }
        }
        if ($null -eq $target) { throw 'Связанная сделка не найдена в разделе "Контроль закупки".' }
...
$btnNavBitrix.Add_Click({ Show-AppPage 'Bitrix'; if ($bitrixGrid.Rows.Count -eq 0) { $btnBitrixRefresh.PerformClick() } })
    ...
    $priceSearchGrid.Add_CellDoubleClick({
        ...
        if (-not [string]::IsNullOrWhiteSpace($link) -and $link -match '^https?://') {
            try { Start-Process $link } catch { [Windows.Forms.MessageBox]::Show($_.Exception.Message, 'Поиск цен') | Out-Null }
        }
    })
    $btnPriceSearch.Add_Click({
        try {
            $pn=[string]$txtPriceSearchPn.Text
            if([string]::IsNullOrWhiteSpace($pn)){throw 'Введите PN.'}
            $quantity=0
            ...
            if($priceSearchGrid.Rows.Count -eq 0){[Windows.Forms.MessageBox]::Show('По этой PN результатов не найдено.','Поиск цен')|Out-Null}
        } catch { [Windows.Forms.MessageBox]::Show($_.Exception.Message,'Поиск цен')|Out-Null } finally { $btnPriceSearch.Enabled=$true }
    })
    $btnPriceSearchGpt.Add_Click({
        try {
            ...
            [Windows.Forms.MessageBox]::Show('Запрос скопирован в буфер обмена.','Поиск в GPT')|Out-Null
    ...
    $btnNewReminder.Add_Click({
        try {
            $title = Show-TextInputDialog 'Новое напоминание' 'Что нужно сделать?' ''
            if ([string]::IsNullOrWhiteSpace($title)) { return }
            $due = Show-TextInputDialog 'Новое напоминание' 'Дата (можно пусто, формат дд.мм.гггг)' ''
    ...
    $btnImportGlobalist.Add_Click({
        try {
            ...
            [Windows.Forms.MessageBox]::Show(('Загружено квот Globalist: {0}' -f $count), 'Globalist') | Out-Null
        } catch {
    ...
    $btnImportRrfq.Add_Click({
        try {
            ...
            Refresh-PurchaseAll
            [Windows.Forms.MessageBox]::Show("Импортировано поставщиков: $($supplierNames.Count)", 'Импорт из RRFQ') | Out-Null
        } catch {
    ...
    function Confirm-PurchaseRrfqQuotes {
        ...
        $message = [string]::Join("`r`n", @($lines))
        $result = [Windows.Forms.MessageBox]::Show($Owner, $message, 'Квоты из RRFQ - добавить в базу?', [Windows.Forms.MessageBoxButtons]::YesNo, [Windows.Forms.MessageBoxIcon]::Question)
    ...
    function Add-PurchaseDocumentsFromPaths {
        ...
        if ($documentType -eq 'RRFQ' -and $rrfqFiles.Count -gt 0) {
            try {
                ...
                foreach ($rrfqFile in @($rrfqFiles)) {
                    $supplierName = Get-SupplierFromFileName $rrfqFile
                    if ([string]::IsNullOrWhiteSpace($supplierName)) { $supplierName = [IO.Path]::GetFileNameWithoutExtension($rrfqFile) }
                    foreach ($quote in @(Get-QuotesFromWorkbook $rrfqFile $supplierName)) {
                        [void]$parsedQuotes.Add($quote)
                    }
                }
                if ($parsedQuotes.Count -gt 0) {
                    $quoteAnalysis = [pscustomobject]@{ RfqPath = [string]::Join('; ', @($rrfqFiles)); Suppliers = @(); RfqRows = @(); Quotes = @($parsedQuotes); Decisions = @(); Priority = 'Price' }
                    if (Confirm-PurchaseRrfqQuotes @($parsedQuotes) @($rrfqFiles) $form) {
                        [void](Save-QuoteHistory $quoteAnalysis 'Импорт из документов сделки')
                        [void]$messages.Add(('В базу квот добавлено: {0}' -f $parsedQuotes.Count))
                    } else { [void]$messages.Add('Импорт квот отменен пользователем.') }
                } else { [void]$messages.Add('В RRFQ не найдено квот.') }
